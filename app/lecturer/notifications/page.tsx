...
export default function LecturerNotificationsPage() {
  ...
  useEffect(() => { fetch("/api/notifications?limit=50").then(async (response) => { if (!response.ok) throw new Error(); return response.json(); }).then((payload) => setItems(payload.data?.items ?? payload.data ?? [])).catch(() => setError("Unable to load notifications.")); }, []);
