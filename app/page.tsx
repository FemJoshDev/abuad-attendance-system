...
const navItems: { label: string; icon: IconName; path: string }[] = [{ label: "Dashboard", icon: "grid", path: "/student/dashboard" }, { label: "My Courses", icon: "book", path: "/courses" }, { label: "Notifications", icon: "bell", path: "/student/notifications" }, { label: "Complaints", icon: "alert", path: "/complaints" }, { label: "Settings", icon: "settings", path: "/settings" }];
function navigate(path: string) { const target = path === "/notifications" ? "/student/notifications" : path; window.history.pushState({}, "", target); window.dispatchEvent(new PopStateEvent("popstate")); }
function Navigation({ mobile = false, onClose, onLogout, currentPath = "/student/dashboard", unreadCount = 0, avatarUrl, name = student.name }: { mobile?: boolean; onClose?: () => void; onLogout: () => void; currentPath?: string; unreadCount?: number; avatarUrl?: string | null; name?: string }) { return <nav className={mobile ? "mobile-nav" : "sidebar"} aria-label="Student navigation">{mobile && <button className="drawer-close" onClick={onClose} aria-label="Close navigation"><Icon name="close" /></button>}<Brand /><div className="nav-links">{navItems.map((item) => <button className={`nav-item ${currentPath === item.path || (item.path === "/settings" && currentPath.startsWith("/settings/")) ? "active" : ""}`} key={item.label} onClick={() => { navigate(item.path); onClose?.(); }} type="button"><Icon name={item.icon} /><span>{item.label}</span>{item.label === "Notifications" && unreadCount > 0 && <em className="nav-badge">{unreadCount}</em>}</button>)}</div><div className="sidebar-bottom"><div className="mini-profile"><Avatar avatarUrl={avatarUrl} name={name} /><div><strong>{name}</strong><span>{student.matricNumber}</span></div></div><button className="logout" onClick={onLogout} type="button"><Icon name="logout" /> Logout</button></div></nav>; }
function Avatar({ large = false, avatarUrl, name = student.name }: { large?: boolean; avatarUrl?: string | null; name?: string }) { const [savedAvatarUrl, setSavedAvatarUrl] = useState(avatarUrl); const [savedName, setSavedName] = useState(name); useEffect(() => { const refresh = () => fetch("/api/settings/profile").then((response) => response.ok ? response.json() : null).then((payload) => { if (payload?.data) { setSavedAvatarUrl(payload.data.avatarUrl); setSavedName(payload.data.fullName); } }).catch(() => undefined); refresh(); window.addEventListener("avatar-updated", refresh); return () => window.removeEventListener("avatar-updated", refresh); }, []); const initials = savedName.split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase(); return savedAvatarUrl ? <img className={`avatar ${large ? "avatar-large" : ""}`} src={savedAvatarUrl} alt={`${savedName} profile`} /> : <div className={`avatar ${large ? "avatar-large" : ""}`} aria-label={`${savedName} profile photo`}>{initials}</div>; }
async function uploadProfilePhoto(file: File) { if (!/[.](jpg|jpeg|png|webp)$/i.test(file.name) || file.size > 5 * 1024 * 1024) throw new Error("Choose a JPG, PNG, or WebP image smaller than 5 MB."); const body = new FormData(); body.append("file", file); const response = await fetch("/api/settings/profile/avatar", { method: "POST", body }); if (!response.ok) throw new Error("Unable to save profile photo."); window.dispatchEvent(new Event("avatar-updated")); }
function ProfileCard({ profile }: { profile?: DashboardApiData["student"] }) { if (!profile) return null; return <section className="profile-card card"><div className="profile-top"><div><p className="eyebrow">STUDENT PROFILE</p><h2>{profile.name}</h2><p className="matric">Matric No: {profile.matricNumber ?? "Not provided"}</p></div><button className="avatar-button" type="button" aria-label="Change profile image"><Avatar large avatarUrl={profile.avatarUrl} name={profile.name} /><span>Change photo</span></button></div><div className="profile-details"><div><span>Student</span><strong>{profile.email}</strong></div><div><span>Account</span><strong>Active</strong></div></div></section>; }
...
function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    const bellButtons = Array.from(document.querySelectorAll<HTMLButtonElement>(".header-notification, .mobile-header .icon-button:last-child"));
    const onBellClick = () => navigate("/student/notifications");
    bellButtons.forEach((button) => button.addEventListener("click", onBellClick));
    const photoButtons = Array.from(document.querySelectorAll<HTMLButtonElement>(".avatar-button"));
    const onPhotoClick = () => { const input = document.createElement("input"); input.type = "file"; input.accept = "image/jpeg,image/png,image/webp"; input.onchange = () => { const file = input.files?.[0]; if (file) uploadProfilePhoto(file).catch(() => undefined); }; input.click(); };
    photoButtons.forEach((button) => button.addEventListener("click", onPhotoClick));
    return () => { bellButtons.forEach((button) => button.removeEventListener("click", onBellClick)); photoButtons.forEach((button) => button.removeEventListener("click", onPhotoClick)); };
  }, []);
...
export default function Home() {
  ...
  if (!path) return null;
  if (!session && (path === "/" || !["/student/dashboard", "/courses", "/notifications", "/student/notifications", "/complaints", "/settings", "/settings/email", "/settings/authentication", "/settings/preferences"].includes(path))) return <LoginPage onLogin={goToDashboard} />;
  ...
  return <AppFrame path={path} unreadCount={unreadCount} onLogout={logout}>{path === "/courses" ? <CoursesPage /> : (path === "/notifications" || path === "/student/notifications") ? <NotificationsPage items={noticeItems} setItems={setNoticeItems} setUnreadCount={setUnreadCount} /> : path === "/complaints" ? <ComplaintPanel /> : <SettingsPanel path={path} theme={theme} onThemeChange={setTheme} />}</AppFrame>;
}
