...
export async function POST(request: Request) {
  ...
  try { return NextResponse.json({ success: true, data: await createAdminCourse({ courseCode, courseTitle, description: typeof body.description === "string" ? body.description.trim() : undefined, unit: typeof body.unit === "number" && Number.isInteger(body.unit) ? body.unit : undefined, department: typeof body.department === "string" ? body.department.trim() : undefined, level: typeof body.level === "string" ? body.level.trim() : undefined, semester: typeof body.semester === "string" ? body.semester : undefined, academicSession: typeof body.academicSession === "string" ? body.academicSession : undefined, isActive: body.isActive !== false }) }, { status: 201 }); } catch { return NextResponse.json({ success: false, error: "Unable to create course. Course code may already exist." }, { status: 409 }); }
}
