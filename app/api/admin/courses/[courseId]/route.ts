...
export async function PATCH(request: Request, { params }: { params: Promise<{ courseId: string }> }) {
  ...
  const data: Partial<{ courseCode: string; courseTitle: string; description: string; unit: number; department: string; level: string; semester: string; academicSession: string; isActive: boolean }> = {};
